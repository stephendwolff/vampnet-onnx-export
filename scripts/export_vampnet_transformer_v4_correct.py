#!/usr/bin/env python3
"""
Export VampNet transformer V4 with CORRECT embedding architecture.
This version uses the proper latent_dim=8 embeddings like the original VampNet.
"""
...
class VampNetTransformerV4Correct(nn.Module):
    """
    VampNet transformer with CORRECT embedding architecture.
    Uses latent_dim=8 embeddings that are projected to d_model.
    """
    ...
    def __init__(self,
                 ...
                 vocab_size: int = 1024,
                 latent_dim: int = 8,  # The KEY difference!
                 d_model: int = 1280,
                 n_heads: int = 20,
                 n_layers: int = 20,
                 dropout: float = 0.0,
        ...
        # Positional encoding
        self.pos_encoding = nn.Parameter(torch.zeros(1, 2048, d_model))
    ...
    def forward(self, codes, mask=None):
        """
        Forward pass with correct embedding.
        Returns logits for proper sampling.
        """
...
def transfer_weights_v4(vampnet_checkpoint, model, model_type="coarse"):
    """Transfer weights from VampNet to our correct architecture."""
    ...
    # 2. Transfer positional encoding
    if hasattr(vampnet, 'positional_encoder'):
        pos_enc = vampnet.positional_encoder.pe.weight.data
        model.pos_encoding.data[:, :pos_enc.shape[1], :] = pos_enc.transpose(0, 1)
    ...
    for i, (onnx_layer, vamp_layer) in enumerate(zip(model.layers, vampnet.transformer.layers)):
        ...
        # FiLM - VampNet uses film_1 and film_3
        # Check if FiLM has input_dim > 0 (if 0, it's a no-op)
        if vamp_layer.film_3.input_dim > 0:
            ...
            # FiLM is a no-op, set to identity transform
            # gamma = 1, beta = 0 (which are the default values in SimpleFiLM init)
            pass
    ...
    # Extract from classifier - it's a SequentialWithFiLM with layers
    classifier_weight = vampnet.classifier.layers[0].weight.data.squeeze(-1)  # [vocab_size * n_predict, d_model]
    ...
    for i in range(n_output_codebooks):
        ...
        # Note: VampNet uses vocab_size, we use vocab_size+1
        vamp_start = i * vampnet.vocab_size
        vamp_end = (i + 1) * vampnet.vocab_size
        
        # Transfer the weights (no transpose needed - Conv1d and Linear have same layout after squeeze)
        model.output_projs[i].weight.data[:vampnet.vocab_size] = classifier_weight[vamp_start:vamp_end]
        # Initialize the mask token weights
...
def export_model_v4(
    ...
    """Export model with correct architecture."""
    
    print(f"\n{'='*60}")
    print(f"Exporting {model_type.upper()} Model V4 with CORRECT Architecture")
    print(f"{'='*60}")
    ...
    if model_type == "coarse":
        ...
        default_output = "onnx_models_fixed/coarse_v4_correct.onnx"
    else:  # c2f
        config = {
            'n_codebooks': 14,
        ...
        default_output = "onnx_models_fixed/c2f_v4_correct.onnx"
    ...
    # Create model
    model = VampNetTransformerV4Correct(**config)
    
    # Transfer weights
    if Path(checkpoint_path).exists():
        transfer_weights_v4(checkpoint_path, model, model_type)
    ...
    torch.save(model.state_dict(), output_path.replace('.onnx', '_weights.pth'))
...
if __name__ == "__main__":
    # Export both models
    export_model_v4("coarse")
    export_model_v4("c2f")
    
    # Clean up
    import os
    os.remove("codebook_embedding_correct.onnx")
    
    print("\n" + "="*60)
    print("V4 MODELS WITH CORRECT ARCHITECTURE EXPORTED!")
    print("These use latent_dim=8 embeddings like the original VampNet")
    print("="*60)