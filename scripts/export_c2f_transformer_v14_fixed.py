#!/usr/bin/env python3
"""
Export C2F (Coarse-to-Fine) Transformer V14 - Fixed FFN dimension.
Fixes the NaN issue by using correct 4x expansion in FFN.
"""
...
class C2FTransformerV14(nn.Module):
    """C2F Transformer V14 - Fixed FFN dimension to prevent NaN."""
...
def transfer_weights_c2f_v14(checkpoint_path, model, codec_path):
    """Transfer weights from VampNet C2F checkpoint to our model."""
    print("Transferring weights to C2F V14 model...")
...
def main():
    print("="*80)
    print("EXPORTING C2F TRANSFORMER V14 (FIXED FFN)")
    print("="*80)
    ...
    with torch.no_grad():
        ...
        print(f"Expected shape: [1, {model.vocab_size}, {seq_len * model.n_predict_codebooks}]")
    ...
    torch.onnx.export(
        model,
        dummy_latents,
        "vampnet_c2f_transformer_v14.onnx",
        input_names=['latents'],
        output_names=['logits'],
        dynamic_axes={
    ...
    print("✓ Exported to vampnet_c2f_transformer_v14.onnx")
    ...
    }, 'vampnet_c2f_transformer_v14.pth')
    
    print("✓ Saved model to vampnet_c2f_transformer_v14.pth")
    
    # Test ONNX model
    print("\nTesting ONNX model...")
    import onnxruntime as ort
    ort_session = ort.InferenceSession("vampnet_c2f_transformer_v14.onnx")
    ...
    # Compare outputs
    if not (torch.isnan(output).any() or np.isnan(onnx_output).any()):
    ...
    with torch.no_grad():
        ...
        if not (torch.isnan(vampnet_output).any() or torch.isnan(output).any()):
            ...
            diff = torch.abs(vampnet_output - output).max()
            print(f"Max difference: {diff:.6f}")
    
    print("\n✅ C2F V14 export complete!")
    print("="*80)
