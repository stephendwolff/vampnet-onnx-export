#!/usr/bin/env python3
"""
Export C2F (Coarse-to-Fine) Transformer with V12 architecture.
Fixed to match VampNet's actual output format.
"""
...
class VampNetEmbeddingLayer(nn.Module):
    """Embedding layer that processes codes directly."""
    def __init__(self, n_codebooks: int, vocab_size: int, d_model: int, 
        ...
        # Embedding layers for each codebook
        self.embeddings = nn.ModuleList([
            nn.Embedding(vocab_size + 1, d_model // n_codebooks)  # +1 for mask token
    ...
    def forward(self, codes):
        """
        Args:
            codes: [batch, n_codebooks, seq_len] - discrete codes
        Returns:
        ...
        for i, embedding in enumerate(self.embeddings):
            ...
            cb_embedded = embedding(cb_codes)  # [batch, seq_len, d_model // n_codebooks]
            embedded.append(cb_embedded)
        
        # Concatenate embeddings
        embedded = torch.cat(embedded, dim=-1)  # [batch, seq_len, d_model]
...
class C2FTransformerV12(nn.Module):
    """C2F Transformer with V12 architecture - matches VampNet's output format."""
    ...
    def __init__(self, 
        ...
        # Embedding layer - expects codes
        self.embedding = VampNetEmbeddingLayer(n_codebooks, vocab_size, d_model, mask_token)
    ...
    def forward(self, codes):
        ...
        # Embedding
        x = self.embedding(codes)  # [batch, d_model, seq_len]
...
def transfer_weights_c2f_v12(checkpoint_path, model, codec_path):
    """Transfer weights from VampNet C2F checkpoint to our model."""
    print("Transferring weights to C2F V12 model...")
    ...
    with torch.no_grad():
        # VampNet's embedding is more complex, we need to extract the relevant parts
        for i in range(model.n_codebooks):
            # Get the embedding weight for this codebook
            vampnet_weight = vampnet.embedding.latent_positions.emb[i].weight.data
            model.embedding.embeddings[i].weight.data = vampnet_weight.clone()
    ...
    with torch.no_grad():
        # VampNet has a single Conv1d classifier
...
def main():
    print("="*80)
    print("EXPORTING C2F TRANSFORMER V12 (FIXED)")
    print("="*80)
    
    # Create model
    model = C2FTransformerV12()
    print(f"\nModel configuration:")
    ...
    # Transfer weights
    transfer_weights_c2f_v12("models/vampnet/c2f.pth", model, "models/vampnet/codec.pth")
    
    # Test model
    print("\nTesting model...")
    model.eval()
    with torch.no_grad():
        # Test input: [batch, n_codebooks, seq_len]
        test_codes = torch.randint(0, 1024, (1, 14, 10))  # Small test
        output = model(test_codes)
        print(f"Input shape: {test_codes.shape}")
        print(f"Output shape: {output.shape}")
        print(f"Expected shape: [1, {model.n_predict_codebooks}, {10 * model.n_predict_codebooks}, {model.vocab_size}]")
    
    # Export to ONNX
    print("\nExporting to ONNX...")
    dummy_input = torch.randint(0, 1024, (1, 14, 100))
    
    torch.onnx.export(
        model,
        dummy_input,
        "vampnet_c2f_transformer_v12.onnx",
        input_names=['codes'],
        output_names=['logits'],
        dynamic_axes={
    ...
    print("✓ Exported to vampnet_c2f_transformer_v12.onnx")
    ...
    }, 'vampnet_c2f_transformer_v12.pth')
    
    print("✓ Saved model to vampnet_c2f_transformer_v12.pth")
    
    # Test ONNX model
    print("\nTesting ONNX model...")
    import onnxruntime as ort
    ort_session = ort.InferenceSession("vampnet_c2f_transformer_v12.onnx")
    
    onnx_output = ort_session.run(None, {'codes': test_codes.numpy()})[0]
